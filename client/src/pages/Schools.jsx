...
const TYPES = { ecde:'ECDE Centre', primary:'Primary School', secondary:'Secondary School', mixed:'Mixed (Pri+Sec)', tertiary:'Tertiary/College' };
const CURRICULA = { cbc:'CBC', '844':'8-4-4', both:'Both (Transition)' };

function SchoolForm({ initial, onSave, onClose }) {
  const [form, setForm] = useState({ name:'', location:'', type:'primary', curriculum:'cbc', contact_name:'', contact_phone:'', ...initial });
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="form-row">
        ...
        <div className="form-group"><label className="form-label">School Type</label>
      ...
      <div className="form-row">
        ...
        <div className="form-group"><label className="form-label">Principal / Contact</label>
          <input className="form-input" value={form.contact_name} onChange={e=>set('contact_name',e.target.value)} placeholder="Mr. Kamau" />
        </div>
      </div>
      <div className="form-group"><label className="form-label">Phone</label>
        <input className="form-input" value={form.contact_phone} onChange={e=>set('contact_phone',e.target.value)} placeholder="+254 7xx xxx xxx" />
      </div>
      <div className="flex-center gap-8" style={{justifyContent:'flex-end',marginTop:8}}>
        <button type="button" className="btn btn-secondary" onClick={onClose}>Cancel</button>
        <button type="submit" className="btn btn-primary" disabled={saving}>{saving?'Saving…':'Save School'}</button>
      </div>
    </form>
  );
}
...
export default function Schools() {
  ...
  async function load() {
    try { const d = await api.getSchools(); setSchools(d); }
  ...
  return (
    <div>
      <div className="flex-between mb-20">
        <div>
          <div className="section-title" style={{fontSize:15,fontWeight:700}}>Partner Schools</div>
          <div className="text-sm text-muted">{schools.length} school{schools.length!==1?'s':''}</div>
        </div>
          ...
          {schools.map(sch => (
            <div key={sch.id} className="card">
              <div className="flex-between mb-12">
                <div className="stat-icon blue" style={{margin:0}}><School size={17} strokeWidth={2} /></div>
                <div className="flex-center gap-6">
                  <button className="btn btn-secondary btn-sm" onClick={()=>setModal(sch)}><Pencil size={12}/> Edit</button>
                  <button className="btn btn-danger btn-sm" onClick={()=>setDelId(sch.id)}><Trash2 size={12}/> Del</button>
                </div>
              </div>
              <div style={{fontSize:15,fontWeight:800,marginBottom:3}}>{sch.name}</div>
              <div className="text-sm text-muted mb-8">{sch.location} · {TYPES[sch.type]||sch.type}</div>
              <span className="badge badge-blue mb-12">{CURRICULA[sch.curriculum]||sch.curriculum}</span>
      ...
      <Modal open={!!modal} onClose={()=>setModal(null)} title={modal?.id ? 'Edit School' : 'Add School'}>
        <SchoolForm initial={modal?.id ? modal : {}} onSave={saveSchool} onClose={()=>setModal(null)} />
      </Modal>
